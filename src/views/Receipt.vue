            ...
            <el-table-column>
              <template slot-scope="scope">
                <div class="remart_wrap font_wrap">
                  ...
                  ><span class="rmb_txt"
                    >{{ scope.row.PJMX_KPBZ
                    }}<em v-if="scope.row.billRemarks != ''"
                      ><br />{{ scope.row.billRemarks }}</em
                    ></span
                  >
                </div>
              </template>
...
<script>
import { billPrint, updatePrintTimes } from "@/api/FeeManagement/TicketBill"; //票据打印
export default {
  data() {
    return {
      ReceiptTab: "",
  ...
  methods: {
    ...
    getReciptData() {
      billPrint(this.$route.query.PJLYMX_ZJ, this.$route.query.X_Token).then(
        (response) => {
          ...
          this.tabledata2[0].PJMX_KPBZ = response.data.PJMX_KPBZ; //备注
          this.tabledata2[0].billRemarks = response.data.billRemarks;
        }
      );
    },
    //收据打印

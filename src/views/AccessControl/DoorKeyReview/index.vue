<template>
  <div class="container DoorKeyReview">
      ...
      <section>
        ...
        >
          ...
          <el-table-column
            ...
            width="160"
          ></el-table-column>
          <el-table-column label="门禁卡" width="160">
            <template slot-scope="scope">
              {{
                scope.row.RYMJQX_SFKTMJK && scope.row.RYMJQX_MJKSQZT == 1
                  ? "已开通"
                  : scope.row.RYMJQX_SFKTMJK && scope.row.RYMJQX_MJKSQZT == 0
                  ? "申请中"
                  : !scope.row.RYMJQX_SFKTMJK
                  ? "未开通"
                  : "未开通"
              }}
            </template>
          </el-table-column>
          <el-table-column label="动态密码" width="160">
            <template slot-scope="scope">
              {{
                scope.row.RYMJQX_SFKTDX && scope.row.RYMJQX_DXSQZT == 1
                  ? "已开通"
                  : scope.row.RYMJQX_SFKTDX && scope.row.RYMJQX_DXSQZT == 0
                  ? "申请中"
                  : !scope.row.RYMJQX_SFKTDX
                  ? "未开通"
                  : "未开通"
              }}
            </template>
          </el-table-column>
          <el-table-column label="扫码开门" width="160">
            <template slot-scope="scope">
              {{
                scope.row.RYMJQX_SFKTEWM && scope.row.RYMJQX_EWMSQZT == 1
                  ? "已开通"
                  : scope.row.RYMJQX_SFKTEWM && scope.row.RYMJQX_EWMSQZT == 0
                  ? "申请中"
                  : !scope.row.RYMJQX_SFKTEWM
                  ? "未开通"
                  : "未开通"
              }}
            </template>
          </el-table-column>
          <el-table-column label="人脸识别" width="160">
            <template slot-scope="scope">
              {{
                scope.row.RYMJQX_SFKTRL && scope.row.RYMJQX_RLSQZT == 1
                  ? "已开通"
                  : scope.row.RYMJQX_SFKTRL && scope.row.RYMJQX_RLSQZT == 0
                  ? "申请中"
                  : !scope.row.RYMJQX_SFKTRL
                  ? "未开通"
                  : "未开通"
              }}
            </template>
...
<script>
import {
  queryEntranceGuardKeyApi,
  updateEntranceGuardKeyApi,
...
export default {
  ...
  methods: {
    getTableList() {
      //列表数据查询
      queryEntranceGuardKeyApi(this.tableParameter).then((response) => {
    ...
    passClick() {
      this.$confirm("是否通过所选项？", "提示", {
        ...
        .then(() => {
          updateEntranceGuardKeyApi(this.detailRow, 1).then((response) => {
            this.radio = ""; //选择后清空选中表格按钮
            this.detailRow = {}; //选择后清空选中表格数据
            this.getTableList();
          });
    ...
    revokeClick() {
      ...
      this.$confirm("是否撤销所选项？", "提示", {
        ...
        .then(() => {
          deleteEntranceGuardKeyApi(this.detailRow, 1).then((response) => {
            this.radio = ""; //选择后清空选中表格按钮
            this.detailRow = {}; //选择后清空选中表格数据
            this.getTableList();
          });

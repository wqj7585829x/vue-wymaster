  ...
  methods: {
    ...
    getGuestData() {
      ...
      queryGuestData(this.FKGX_ZJ, this.selectType).then((response) => {
        ...
        this.formList.old_paymentPle = response.KHDA_KHMC;
      });
    },
    ...
    submit() {
      ...
      this.formList.isChanged =
        this.formList.old_paymentPle == this.formList.paymentPle ? 0 : 1;
      var list = [];
      //确定收清
      this.$refs.ruleForm.validate((valid) => {
        if (valid) {
          ...
          submitFeeCollectionHome(this.formList, list).then((response) => {

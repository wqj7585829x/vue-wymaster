...
export default {
  props: ["obsoleteShow"], //显示隐藏资源名称与禁用客户信息,父子传递信息,收费项目
  data() {
    return {
  ...
  methods: {
    getObjsoleteRow(data, ZFR, type) {
      ...
      if (type) this.formList.typeValue = 1;
    },
    sureForm() {
      var _this = this;
      this.$refs.tableForm.validate((valid) => {
        if (valid) {
          ...
          confirmCancellation(_this.formList).then((response) => {

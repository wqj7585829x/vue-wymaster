...
export default {
  ...
  methods: {
    getTableRow(data, type) {
      ...
      this.arrname = [];
      if (
        this.formList.kelectronicDocument &&
        this.formList.kelectronicDocumentName
      ) {
        this.arrlist = this.formList.kelectronicDocument.split(";");
        this.arrname = this.formList.kelectronicDocumentName.split(";");
      }
    },
    cancelForm() {
      this.fileList = [];

...
export default {
  ...
  data() {
    return {
      ...
      configData: [
        ...
        { content: "调整类型", width: 150, field: "adjustmentType" },
  ...
  methods: {
    ...
    slectRow(val, row) {
      //val代表所有选中的行对象数组，row当前选中行对象
      if (val.indexOf(row) > -1) {
        //当前行选中，则之前的费用包括自己选中
        this.tableData.forEach((item) => {
          if (row.FYSQ_SSNY >= item.FYSQ_SSNY) {
            //FYSQ_SSNY选中日期大于等于
            this.$refs.NotResingleTables.toggleRowSelection([
        ...
        //当前行取消选中，则当前的费用以及之后取消，之前不取消,
        this.tableData.forEach((item) => {
          if (row.FYSQ_SSNY <= item.FYSQ_SSNY) {
            //取消的时候，前面日期费用不取消，后面日期取消
            this.$refs.NotResingleTables.toggleRowSelection([
              { row: item, selected: false },
            ]);
            this.multipleSelection.forEach((its, index) => {
              if (its == item) {
      ...
      this.multipleSelection = [...new Set(this.multipleSelection)]; //去重
    },
      ...
      if (!val.isChildren) {
        ...
        this.childrenArr.forEach((item) => {
          ...
          if (item.pid == val.FYSQ_ZJ && item.FYSQ_XPKPZT == 1) {
            //判断选中的主键如果等于子级主键
            item.isCheck = val.isCheck; //选中不选中设置
            if (item.isCheck) {
              //选中/不选中  改变对应子级的isAdd
              isRecord++;
            } else {

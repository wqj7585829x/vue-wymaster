<template>
  ...
  >
    ...
    >
      ...
      <el-row :gutter="50">
        ...
        <el-col :span="12">
          <el-form-item label="纳税人识别号：">
            <el-input v-model="formList.KHDA_NSRSBH" readonly></el-input>
          </el-form-item>
        </el-col>
        ...
        <el-col :span="12">
          <el-form-item label="地址、电话：">
            <el-input v-model="formList.KHDA_NSRDZDH" readonly></el-input>
          </el-form-item>
        </el-col>
        ...
        <el-col :span="12">
          <el-form-item label="开户行及账号：">
            <el-input v-model="formList.KHDA_KHHJZH" readonly></el-input>
          </el-form-item>
        </el-col>

        <el-col :span="12">
          <el-form-item label="增值税发票分类：">
            <el-select
        ...
        <el-col :span="12" v-if="formList.LXNO == 101 && sess_kk_ktzzfp == 1">
      ...
      <el-row :gutter="30">
        <div class="bzRow">
          <span class="bztitle">备注</span>
          <span class="bzcontent"
            ><el-input
              type="textarea"
              :row="3"
              placeholder="请输入备注内容"
      ...
      <el-row :gutter="30">
        ...
        <el-col :span="6">
          <el-form-item label="复核人：" label-width="75px">
            <span><el-input v-model="formList.checker" /></span>
          </el-form-item>
        </el-col>
...
// import { isGetInvoiceData } from "@/api/BasicManagement/CustomerFile";
export default {
  ...
  watch: {
    showTicket() {
      ...
      if (this.visible) {
        ...
        this.tableHeight = window.innerHeight - 400; //window.innerHeight当前可视窗口高度
      }
    },
  },
  mounted() {
    // isGetInvoiceData().then(response => {
    //   this.isGetInvoiceData = response.isGetInvoiceData
    // });
  },
  methods: {
    nsrchange(index) {
      ...
      if (typeof index == "number") {
        ...
        //this.$set(this.formList, 'KHDA_KHMC', this.nsrtable[index].invoice_object ? this.nsrtable[index].invoice_object : '')
        this.$set(
          this.formList,
          "KHDA_NSRSBH",
    ...
    getcheckInvoice() {
      ...
      checkInvoice().then((response) => {
        //只有勾选了金税盘，纸质发票才可以打印预览
        this.sess_kk_ktzzfp = response.sess_kk_ktzzfp; //sess_kk_ktzzfp中Z端配置了开启金税盘标识
        if (this.sess_kk_ktzzfp == 1 && this.isCheckTax) {
          //isCheckTax勾选了税盘号,保存标识  如果没有勾选税盘号 则票据列表纸质发票不可打印预览 1可以打印  0不能打印
          this.$set(this.formList, "PJLYMX_SFGXJSP", 1);
        } else {
          this.$set(this.formList, "PJLYMX_SFGXJSP", 0);
        }
      });
    },
    getGroupInfoByJT() {
    ...
    printChange(val) {
      ...
      if (this.formList.LXNO == 101) {
        ...
        this.$set(this.formList, "PJLYMX_SFGXJSP", "");
      }
    },
    getCientData(mulArr) {
      //多选开票
    ...
    getPayPerson() {
      ...
      queryPayPerson(this.formList).then((response) => {
        ...
        if (response.personAndTime) {
          ...
          this.formList.KHDA_KHMC = response.personAndTime[0].KHDA_KHMC; //客户名称分离出来  不从纳税人名称中获取
        } else {
          this.formList.JKR = response[0].JKR;
          this.formList.RZSJ = response[0].RZSJ;
          this.formList.KHDA_KHMC = response[0].KHDA_KHMC;
        }
    ...
    getTaxInfo() {
      ...
      queryInvoiceInformation({
        ...
        if (this.formList.JKR) {
          //纳税人信息添加交款人
          if (this.formList.JKR.indexOf(",") > -1) {
            var jkrSplit = this.formList.JKR.split(",");
            jkrSplit.forEach((item) => {
              var obj = {};
              obj.invoice_nsrmc = item;
              this.nsrtable.push(obj);
            });
          } else {
            this.nsrtable.push({ invoice_nsrmc: this.formList.JKR });
          }
        }
        if (this.nsrtable.length > 0) {
          this.formList.nsrindex = 0;
          this.formList.KHDA_NSRMC =
            this.nsrtable[0].invoice_nsrmc != undefined
              ? this.nsrtable[0].invoice_nsrmc
              : "";
          // this.formList.KHDA_KHMC = this.nsrtable[0].invoice_object != undefined ? this.nsrtable[0].invoice_object : ''
          this.formList.KHDA_NSRSBH =
            this.nsrtable[0].invoice_nsrsbh != undefined
              ? this.nsrtable[0].invoice_nsrsbh
              : "";
          this.formList.KHDA_NSRDZDH =
            this.nsrtable[0].invoice_nsrdzdh != undefined
              ? this.nsrtable[0].invoice_nsrdzdh
              : "";
          this.formList.KHDA_KHHJZH =
            this.nsrtable[0].invoice_khhjzh != undefined
              ? this.nsrtable[0].invoice_khhjzh
              : "";
        } else {
          this.nsrtable = [];
      ...
      //     if (this.hometype) {
      //       this.formList.KHDA_NSRMC = this.nsrform.invoice_nsrmc
      //       this.formList.KHDA_KHMC = this.nsrform.invoice_object
      //       this.formList.KHDA_NSRSBH = this.nsrform.invoice_nsrsbh
      //       this.formList.KHDA_NSRDZDH = this.nsrform.invoice_nsrdzdh
      //       this.formList.KHDA_KHHJZH = this.nsrform.invoice_khhjzh
      //     } else {
      //       if (response.taxpayerInformation.length > 0) {
      //         if (response.taxpayerInformation[0].KHDA_NSRMC) {//如果纳税人名称不为空则获取纳税人名称  否则获取交款人
      //           this.formList.KHDA_NSRMC = response.taxpayerInformation[0].KHDA_NSRMC;
      //         } else {
      //           this.formList.KHDA_NSRMC = this.formList.JKR;
      //         }
      //         this.formList.KHDA_KHMC = response.taxpayerInformation[0].KHDA_KHMC;

      //         this.formList.KHDA_NSRSBH = response.taxpayerInformation[0].KHDA_NSRSBH;
      //         this.formList.KHDA_NSRDZDH = response.taxpayerInformation[0].KHDA_NSRDZDH;
      //         this.formList.KHDA_KHHJZH = response.taxpayerInformation[0].KHDA_KHHJZH;
      //       }
      //     }

      //     this.infoselleraddressphone = response.configuration.kk_dz + " " + response.configuration.kk_dh;//销售方地址，电话
      //     this.infosellerbankaccount = response.configuration.kk_khhdz + " " + response.configuration.kk_yhzh;//销售方开户行及账号
      //   });
      // }
    },
    getMakeBillBelongCategory() {
    ...
    changeFun(val) {
      ...
      this.$nextTick(() => {
        //重置清空数据
        this.$set(this.formList, "remark", "");
        this.formList.SKMX_SKFYMC = "";
        this.formList.SKMX_SKZY = "";
      });
      this.multipleSelection = val;
      this.multipleSelection.forEach((item) => {
        this.formList.SKMX_SKFYMC += item.SKMX_SKFYMC + "`";
        this.formList.SKMX_SKZY += item.SKMX_SKZY + "`";
      });
      if (val.length) {
        this.getBZInfo(); //获取备注信息
      }
    },
    getBZInfo() {
      //获取备注信息
      if (this.formList.FKGX_ZJ) {
        getBZInfoApi(this.formList).then((response) => {
          if (response.data) {
            this.$nextTick(() => {
              this.$set(this.formList, "remark", response.data.remarkMessage);
            });
          }
        });
      }
    ...
    OpenCard_kp() {
      ...
      //   // infonotes: this.formList.PJMX_KPBZ == undefined ? "" : this.formList.FJMC + this.formList.PJMX_KPBZ,//备注
      //   infonotes: this.formList.remark, //备注
      //   infoinvoicer: this.userName, //开票人
      //   infochecker: this.formList.checker, //复核人

        ...
        <el-row :gutter="30">
          ...
          <el-col :span="6" style="width: 250px">
            <el-form-item label="收款方式:" label-width="70px">
              <el-select v-model="tableParameter.SFKM_ZJ" clearable>
                <el-option
    ...
    <section class="footer_bottomDetail">
      ...
      >
        ...
        <u-table-column
          ...
          width="120"
        ></u-table-column>
        <u-table-column label="收款金额（含税）" width="150">
          <template slot-scope="scope">
        ...
        <u-table-column
          ...
          width="100"
        ></u-table-column>
        <u-table-column label="金额（不含税）" width="150">
          <template slot-scope="scope">
            {{ formatMoney(scope.row.SKMX_BHSJE) }}
          </template>
        </u-table-column>
        <u-table-column label="开票金额" width="150">
          <template slot-scope="scope">
...
import {
  ...
  exportExcelApi,
  querytPayWay,
  submitCancelApplicationsApi,
} from "@/api/FeeManagement/CollectionCredentials_new"; //收款凭证Api
  ...
  methods: {
        ...
        (response) => {
          ...
          if (this.tableData.length > 0) {
            if (this.selectType == 1) {
              ...
              this.getisCancelProve(this.tableRow); //查询是否可以作废
              /**判断是否可以开票 end*/
    ...
    getisCancelProve(obj) {
      ...
      isCancelProve(obj.SKPZ_ZJ).then((response) => {
        this.disabledCancel = response.isCancelProve == 1 ? false : true;
        this.disabledApply = response.isCancelApply == 1 ? false : true;
      });
    },
    ApplyInvalid() {
      //申请作废
      this.$confirm("是否继续此操作?", "提示", {
        ...
        .then(() => {
          ...
          if (this.selectType == 1) {
            ...
            obj = this.multipleSelection[0];
          }
          submitCancelApplicationsApi(obj.SKPZ_ZJ).then(() => {
            this.getDataList(1);
          });
        })
        .catch(() => {});
    ...
    rowClick(row) {
      if (this.selectType == 1) {
        ...
        if (row.SKPZ_YXPZ) {
          //this.disabledCancel = false;
          this.getisCancelProve(row); //判断是否可作废查询
        } else {
          this.titleCancel = "不可作废";
          this.disabledCancel = true;
        }
    ...
    CancellationDialog() {
      //作废弹窗
      if (this.selectType == 1) {
        this.getisExisBill(this.tableRow);
      } else {
        this.getisExisBill(this.multipleSelection[0]);
      }
    },
    TicketDialog() {
      if (this.selectType == 1) {

  ...
  <div class="ReceivablesRegister">
    ...
    <div class="el-title" style="height: 100%; overflow: hidden" ref="elTitle">
      ...
      <div v-show="TreeNavBar == 1">
        ...
        <!--配置延迟加载 用表格显示start-->
        <!-- <section v-if='isLazyLoad == 1' class="guest_wrap" :style="contentStyleObj">
          <el-input v-model="filterguest" placeholder="按房间搜索" class="guest_input" @keyup.enter.native="getReourceList"
            clearable>
            <i slot="suffix" class="el-icon-search" @click="getReourceList"></i>
          </el-input>
          <u-table :data="tableSourceList" border highlight-current-row use-virtual :row-height="30" :height="Uheight"
            @row-click="getTreeData" class="guest_table">
            <u-table-column label="大楼" prop="DLDA_DLMC">
              <template slot-scope="scope">
                <span>{{scope.row.GLQDA_GLQMC}}-{{scope.row.DLDA_DLMC}}</span>
              </template>
            </u-table-column>
            <u-table-column label="房间" prop="FCDM_MC"></u-table-column>
          </u-table>
        </section> -->
        <!--配置延迟加载 用表格显示end-->
      </div>

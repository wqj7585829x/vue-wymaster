<template>
  <div class="NewReport">
    ...
    <!-- 报表标题 -->
    <div class="ReportTitleDIV"><strong>应收调整费用明细报表</strong></div>
    <!--  表begin -->
    ...
    <!--  表 end -->
    <table2Popup :show.sync="show" ref="table2Popup"></table2Popup>
  </div>
</template>
...
import {
  queryAllPayItems,
  queryAdjustmentReceivableDetailsReportApi,
  exportApi,
...
import table2Popup from "./table2Popup";
export default {
  data() {
    return {
      tableData: [], //列表参数
  ...
  methods: {
    popupShow(data, name) {
      this.$refs.table2Popup.getPartitionData({
    ...
    getTableData() {
      ...
      queryAdjustmentReceivableDetailsReportApi(data).then((response) => {

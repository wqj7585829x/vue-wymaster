...
<script>
import { updateNoFinancialNoPass } from "@/api/PublicRepairManager/AuditVouchers";
export default {
  data() {
    return {
  ...
  methods: {
    ...
    sure() {
      this.$refs.ruleForm.validate((valid) => {
        if (valid) {
          updateNoFinancialNoPass(this.formList).then((response) => {

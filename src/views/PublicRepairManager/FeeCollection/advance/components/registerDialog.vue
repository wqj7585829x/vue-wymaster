...
<script>
import { costAdvance } from "@/api/PublicRepairManager/FeeCollection";
export default {
  data() {
    return {
      visible: this.show,
  ...
  methods: {
    ...
    getPartitionData(data, type) {
      ...
      this.maxMoney = (data.WXFA_YZFY - data.WXFA_YYZJE).toFixed(5);
      this.RepairForm = JSON.parse(JSON.stringify(data));
      this.RepairForm = {
        WXFA_ZJ: data.WXFA_ZJ,
        WXFA_YZFY: data.WXFA_YZFY,
        WXFA_YYZJE: data.WXFA_YYZJE,
        FYZC_ZCJE: data.WXFA_YZFY - data.WXFA_YYZJE,
        FYZC_CNR: data.CZR,
        FYZC_XMMC: data.WXFA_XMMC,
        FYZC_LQR: data.CZR,
        FYZC_ZCRQ: time,
        FYZC_ZCLX: "预支",
      };
    ...
    sure() {
      ...
      this.$refs["RepairForm"].validate((valid) => {
        if (valid) {
          costAdvance(_this.RepairForm).then((response) => {

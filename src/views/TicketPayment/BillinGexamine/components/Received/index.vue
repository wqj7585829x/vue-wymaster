  ...
  <div class="container Received">
    ...
    <div class="customedetail">
      ...
      >
        ...
        <el-table-column prop="orderId" label="申请批次号" width="200">
        </el-table-column>
        ...
        <el-table-column prop="wyApplyStatus" label="物业审核状态">
          <template slot-scope="scope">
    ...
    <div class="customedetail">
      ...
      <el-table ref="singleTable1" max-height="200" :data="tableData1" border>
        ...
        <el-table-column prop="SKMX_SKJE" label="收款项目" width="150">
        </el-table-column>
        <el-table-column prop="SKMX_SSNY" label="所属年月" width="200">
        </el-table-column>
        <el-table-column prop="SKMX_SKFS" label="收款方式" width="200">
        </el-table-column>
        <el-table-column prop="SKMX_SKJE" label="金额（含税）" width="200">
        </el-table-column>
        <el-table-column prop="SKMX_SL" label="税率（%）" width="200">
        </el-table-column>
        <el-table-column prop="SKMX_BHSJE" label="金额（不含税）" width="200">
        </el-table-column>
...
import {
  ...
  queryRoomMessage,
  queryClientForRoom,
  queryPaidApplicationForms,
  queryCollectionDetail,
  updatePaidApplyBillSuccess,
  updatePaidApplyBillFail,
  paidInvoice,
  getXpInvoiceApplyCheck,
  xpApplyPayCancellation,
  updateApplyBillCancelReceivedApi,
  checkbillCancelApplyReceivedApi,
...
export default {
  ...
  mounted() {
    ...
    getXpInvoiceApplyCheck().then((response) => {
      this.xpInvoiceApplyCheck = response.data.xpInvoiceApplyCheck;
    });
  },
  methods: {
    checkbox(row) {
    ...
    cancellation() {
      xpApplyPayCancellation({
        billApplyId: this.tableRow.billApplyId,
    ...
    invoice() {
      this.OBshow = true;
      this.$refs.ticketChilren.getData(this.tableRow, 2);
    },
    billdetail() {
      this.show = true;
    ...
    checkbillCancelApply() {
      //取消申请
      this.$confirm("此操作将取消申请应数据, 是否继续?", "提示", {
        confirmButtonText: "是",
        cancelButtonText: "否",
        distinguishCancelAndClose: true,
        type: "warning",
    ...
    rowclick(row) {
      this.tableRow = row.isChildren === 0 ? row : {};
      var data = this.tableData;
      var total = 0;
      for (var i in data) {
        if (
          typeof data[i].list != "undefined" &&
          i < this.tableData.indexOf(row)
        ) {
          total = total + data[i].list.length;
        }
      }

      this.radio =
        row.isChildren === 0 ? this.tableData.indexOf(row) + total : ""; //单行点击单选按钮选中

      this.disablebutton =
        row.wyApplyStatus === 0 && row.isChildren === 0 ? false : true;
      this.disablebuttonCancel =
        ...
        (row.isJtApply === true && row.wyApplyStatus == 3)
          ...
          : true; //是否集团审核否
      this.disableddobill =
        row.status === 1 && row.canInvoice && row.isChildren === 0
          ? false
          : true;
    ...
    getTableData() {
      queryPaidApplicationForms(this.searchList).then((response) => {

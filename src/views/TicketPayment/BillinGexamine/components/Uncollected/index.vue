      ...
      >
        ...
        <el-table-column prop="status" label="状态" width="150">
          <template slot-scope="scope">
            ...
            <!-- <span v-if="scope.row.status===9">作废</span> -->
          </template>
        </el-table-column>
        <el-table-column prop="applyInvoiceState" label="开票状态" width="150">
          <template slot-scope="scope">
            ...
            <span v-if="scope.row.applyInvoiceState === 2">部分开票</span>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="isJtApply" label="是否集团审核" width="200">
          <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    rowclick(row) {
      ...
      this.disablebuttonCancel =
        ...
        (row.isJtApply === true &&
          ...
          row.jtApplyStatus == 0)
          ? false
          : true; //是否集团审核否--否，物业审核要等于2或3才可以取消审核-----//是否集团审核否--是，物业审核要等于2或3，以及集团端要未审核0 才可以取消审核
    ...
    getTableData() {
      queryUnpaidApplicationForms(this.searchList).then((response) => {

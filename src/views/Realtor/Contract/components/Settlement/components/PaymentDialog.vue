...
import { updateLiquidatedDamages } from "@/api/Realtor/Contract";
export default {
  props: ["showPayment"],
  watch: {
    showPayment() {
  ...
  methods: {
    getData(data, type) {
      if (type === 0) {
        ...
        this.PaymentForm.contractId = data.contractId;
      } else {
        this.PaymentForm = JSON.parse(JSON.stringify(data));
        this.title = "修改";
      }
    },
    ...
    queryContractRequest() {
      ...
      queryContractListApi(this.PaymentForm).then((response) => {
        ...
        this.contractChag(this.PaymentForm.contractId);
      });
    },
    inputMouseblur(val) {
      //鼠标离开事件
      ...
      this.$refs.PaymentForm.validate((valid) => {
        if (valid) {
          if (this.title == "新增") {
            ...
            updateLiquidatedDamages(this.PaymentForm).then((response) => {
              if (response.returnStatus == "SUCCESS") {
                this.$emit("RefreshItem");
                this.cancelForm();
              }

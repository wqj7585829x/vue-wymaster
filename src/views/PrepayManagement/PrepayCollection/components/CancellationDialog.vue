    ...
    <el-form label-width="90px" :model="formList" :rules="rules" ref="ruleForm">
      <el-row>
        <el-col :span="24">
          ...
          <el-form-item label="作废日期：" prop="YJPZ_ZFSJ">
            <el-date-picker
              type="date"
              :editable="false"
              placeholder="选择日期"
              v-model="formList.YJPZ_ZFSJ"
              format="yyyy-MM-dd"
              value-format="yyyy-MM-dd"
            ></el-date-picker>
          </el-form-item>
          <el-form-item label="作废原因：" prop="YJPZ_ZFYY">
            <el-input
              type="textarea"
              placeholder=""
              v-model="formList.YJPZ_ZFYY"
            ></el-input>
          </el-form-item>
        </el-col>
      </el-row>
...
import {
  destroyPrepayProve,
  getCancelParameterOfPayingProve,
} from "@/api/PrepayManagement/PrepayCollection"; //收款凭证作废Api
...
export default {
  ...
  methods: {
    getData(FKGX_ZJ, YJPZ_ZJ, data) {
      var pjzj = "";
      var pjlymx = "";
      this.formList.FKGX_ZJ = FKGX_ZJ; //房客关系主键
      this.formList.YJPZ_ZJ = YJPZ_ZJ; //收款凭证主键
      if (data.length > 0) {
        data.forEach((item) => {
          pjzj += item.PJLYMX_ZJ + ",";
      ...
      this.getParmRequest({ YJPZ_ZJ: YJPZ_ZJ }); //查询作废参数
    },
    getParmRequest(val) {
      //查询作废参数
      getCancelParameterOfPayingProve(val).then((response) => {
    ...
    submitForm() {
      ...
      this.$refs.ruleForm.validate((valid) => {
        if (valid) {
          ...
          this.$nextTick(() => {
            destroyPrepayProve(this.formList).then((response) => {

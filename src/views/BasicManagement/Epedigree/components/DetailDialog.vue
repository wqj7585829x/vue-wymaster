  ...
  <div class="DetailDialog">
    ...
    >
      ...
      >
        <el-row :gutter="50">
          ...
          <el-col :span="12">
            <el-form-item label="品牌:" prop="brand">
              <el-input v-model="DetailForm.brand"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-row :gutter="50">
          <el-col :span="12">
            <el-form-item label="型号:" prop="model">
              <el-input v-model="DetailForm.model"></el-input>
            </el-form-item>
          </el-col>

          <el-col :span="12">
            <el-form-item label="数量:" prop="num">
              <el-select v-model="DetailForm.num">
                <el-option
                  v-for="item in 10"
                  :key="item"
                  :label="item"
        ...
        <el-row :gutter="50">
          <el-col :span="18">
            <el-form-item label="入住状况:" prop="check_in_status">
              <el-input
                v-model="DetailForm.check_in_status"
                placeholder="简要描述入住时设施设备大致情况"
...
<script>
import { addDeviceDetail, updateDeviceDetail } from "@/api/Realtor/Contract";
export default {
  data() {
    return {
    ...
    getReportData(data, type) {
      ...
      if (type == 0) {
        ...
        if (data.upload_photo) {
          var pic = data.upload_photo.split(";");
          this.allimg = data.upload_photo.split(";");
          this.deleteimg = [];
          this.fileList = [];
          pic.forEach((item) => {
            var obj = {};
      ...
      this.$refs["DetailForm"].validate((valid) => {
        if (valid) {
          if (this.title === "新增") {
            ...
            addDeviceDetail(this.DetailForm).then((response) => {
            ...
            updateDeviceDetail(this.DetailForm).then((response) => {
              if (response.returnStatus == "SUCCESS") {
                let newobj = JSON.parse(JSON.stringify(_this.DetailForm));
                _this.$emit("RefreshItem");
                _this.cancle();

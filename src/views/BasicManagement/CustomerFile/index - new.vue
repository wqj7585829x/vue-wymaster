  ...
  <div class="el-body CustomerFile">
    ...
    <div class="el-content">
      <div class="tabletop">
        ...
        <div class="button buttonBox">
          ...
          <el-button
            type="primary"
            :disabled="disableddo || !inherittype || tableRow.KHDA_SFKHWJ == ''"
            @click="inheritCus(0)"
    ...
    >
        ...
        <el-row>
          <el-col>
            <el-form-item label="收费对象：">
              <el-select v-model="cusList" filterable clearable>
                <el-option
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="inheritCus(1)">继承参数</el-button>
      </div>
    </el-dialog>
...
import {
  ...
  exportExcelData,
  queryInheritCustomer,
  queryInheritCustomerInfo,
  statusToCurrent,
...
export default {
  ...
  methods: {
    ...
    inheritCus(type) {
      if (type == 0) {
        queryInheritCustomerInfo({
          FJDA_ZJ: this.tableRow.FJDA_ZJ,
          KHDA_ZJ: this.tableRow.KHDA_SFKHWJ,
        }).then((response) => {
          this.$refs.inheritDialog.getPartitionData(
            {
              OLD_FKGX_ZJ: this.tableRow.FKGX_ZJ,
              NEW_FKGX_ZJ: response.data.FKGX_ZJ,
              FKGX_FJWJ: this.tableRow.FJDA_ZJ,
              NEW_KHMC: response.data.KHMC,
              OLD_KHMC: this.tableRow.KHDA_KHMC,
              leaseDate: this.rentTime,
              GLCID: this.searchCustomerFile.GLCID,
              YXDID: this.searchCustomerFile.YXDID,
            },
            0
        ...
        if (!this.cusList) {
          this.$message.warning("没有选择收费对象，请检查！");
          return;
        }
        if (!this.rentTime) {
          this.$message.warning("没有选择退租日期，请检查！");
          return;
        }
        const p = this.options;
        const c = this.cusList;
        const index = p.findIndex((d) => d.FKGX_ZJ === c);

        this.$refs.inheritDialog.getPartitionData(
          {
            OLD_FKGX_ZJ: this.tableRow.FKGX_ZJ,
            NEW_FKGX_ZJ: this.cusList,
            FKGX_FJWJ: this.tableRow.FJDA_ZJ,
            NEW_KHMC: this.options[index].KHDA_KHMC,
            OLD_KHMC: this.tableRow.KHDA_KHMC,
            leaseDate: this.rentTime,
            GLCID: this.searchCustomerFile.GLCID,
            YXDID: this.searchCustomerFile.YXDID,
    ...
    rentCus() {
      ...
      customerRetreatLease({
        ...
        FKGX_TZRQ: this.rentTime,
        KHDA_SFKHWJ: this.options[index].KHDA_ZJ,
        KHDA_ZJ: this.tableRow.KHDA_ZJ,
      }).then((response) => {

      ...
      <div class="content">
        ...
        <div class="search">
          <el-input
            ...
            placeholder="按资源代码定位"
          ></el-input>
          <span v-if="showtype">待选记录笔数:{{ tableindex1.length }}</span>
          <div v-if="!showtype" style="margin-left: 20px; margin-bottom: 5px">
            待选记录笔数:{{ tableindex1.length }}
          </div>
        </div>
        <div class="body">
          <u-table
      ...
      <div class="content">
        ...
        <div class="search">
          <el-input
            ...
            placeholder="按资源代码定位"
          ></el-input>
          <span v-if="showtype">已选记录笔数:{{ tableindex2.length }}</span>
          <div v-if="!showtype" style="margin-left: 20px; margin-bottom: 5px">
            已选记录笔数:{{ tableindex2.length }}
          </div>
        </div>
        <div class="body">
...
import {
  ...
  binding,
  unBinding,
} from "@/api/SharedHydropowerDesign/RelationshipSetting";
...
export default {
  data() {
    return {
      ...
      showtype: true,
      Leftvalue: "", //待选记录输入框内容值
      Rightvalue: "", //已选记录输入框内容值
    };
  },
  components: {
    UTable,
  ...
  computed: {
    tableDataL() {
      ...
      if (Leftvalue) {
        return this.tableData1.filter(function (item) {
          return Object.keys(item).some(function (key) {
            if (
              ...
              return String(item[key]).toLowerCase().indexOf(Leftvalue) > -1;
            }
          });
        });
      }
    ...
    tableDataR() {
      ...
      if (Rightvalue) {
        return this.tableData2.filter(function (item) {
          return Object.keys(item).some(function (key) {
            if (
              key == "ZBJS_FJID" ||
              key == "ZBJS_KHMC" ||
              ...
              return String(item[key]).toLowerCase().indexOf(Rightvalue) > -1;
            }
          });
        });
      }
  ...
  methods: {
    ...
    getqueryPendingResource() {
      queryPendingResource(this.transferForm).then((response) => {
        this.totaltableData1 = response.data;
        this.tableindex1.length = response.data.length;
    ...
    getqueryChildrenTable() {
      queryChildrenTable(this.transferForm).then((response) => {
        this.totaltableData2 = response.data;
        this.tableindex2.length = response.data.length;
    ...
    relativewait() {
      ...
      items.forEach((item) => {
        ZBJS_ZJ.push(item.ZBJS_ZJ);
      }); //选中记录构造接口部分参数LIst
      var _this = this;
      binding({
        FTZB_ZJ: this.transferForm.FTZB_ZJ,
        isAll: 0,
        FCDM_LX: this.transferForm.FCDM_ID,
        FCDM_ID: this.transferForm.FCDM_LX,
        ZBJS_ZJ: ZBJS_ZJ.join(","),
      }).then((response) => {
        //操作完成后数据刷新重置-begin
        this.getqueryPendingResource();
        this.getqueryChildrenTable();
    ...
    relativealready() {
      ...
      items.forEach((item) => {
        ZBFT_ZJ.push(item.ZBFT_ZJ);
      });
      unBinding({
        FTZB_ZJ: this.transferForm.FTZB_ZJ,
        isAll: 0,
        ZBFT_ZJ: ZBFT_ZJ.join(","),
      }).then((response) => {
        this.getqueryPendingResource();
        this.getqueryChildrenTable();
        this.checkboxlist1 = [];

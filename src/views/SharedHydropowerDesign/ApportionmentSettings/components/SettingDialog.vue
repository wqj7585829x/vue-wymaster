    ...
    >
      ...
      >
        ...
        <el-row :gutter="50">
          ...
          <el-col :span="12">
            <el-form-item label="分摊项目:" prop="index">
              <el-select
        ...
        <el-row :gutter="50">
          <el-col :span="12">
            <el-form-item label="表代码:" prop="FTZB_BDM">
              <el-input v-model="TableForm.FTZB_BDM"></el-input>
            </el-form-item>
          </el-col>

          <el-col :span="12">
            <el-form-item label="是否参与差量分摊:" prop="FTZB_SFCYCLFT">
              <el-checkbox v-model="TableForm.FTZB_SFCYCLFT"></el-checkbox>
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-row :gutter="50">
          ...
          <el-col :span="12" v-if="TableForm.FTZB_FTLB != '直接金额分摊'">
            <el-form-item label="计内单价:" prop="FTZB_JNDJ">
              <el-input-number
...
<script>
import { queryApportionedProjectName } from "@/api/FeeManagement/AllocationSettings";
import {
  addTotalTable,
  updTotalTable,
...
export default {
  data() {
    ...
    return {
      ...
      rules: {
        ...
        index: [{ required: true, message: "请输入计内单价", trigger: "blur" }],
  ...
  methods: {
    SFXMchange(index) {
      this.TableForm.FTZB_SFXMMC = this.tableData[index].SFXM_SFXMMC;
      this.TableForm.FTZB_SFXMWJ = this.tableData[index].SFXM_ZJ;
    },
    FTLBchange(val) {
      if (
        val != "直接用量分摊" &&
    ...
    getPartitionData(data, type) {
      if (type == 0) {
        ...
        data.FTZB_FTLB = "";
        data.FTZB_FTBL = 100;
        data.FTZB_SFCYCLFT = true;

        data.FTZB_SJBMC = data.FTZB_ZBMC ? data.FTZB_ZBMC : "";
        data.FTZB_SJBZJ = data.FTZB_ZJ ? data.FTZB_ZJ : "";
        delete data.FTZB_ZBMC;
        delete data.FTZB_ZJ;
      } else {
        this.title = "修改";
      }
      ...
      this.$refs["TableForm"].validate((valid) => {
        if (valid) {
          if (_this.title == "新增总表") {
            ...
            updTotalTable(_this.TableForm).then((response) => {
              if (response.returnStatus == "SUCCESS") {
